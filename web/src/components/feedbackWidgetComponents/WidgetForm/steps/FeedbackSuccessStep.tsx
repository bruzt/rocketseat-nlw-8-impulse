...
interface IProps {
  restartFeedbackType: () => void;
}

export function FeedbackSuccessStep(props: IProps) {
  return (
    <>
      <header>
        <CloseButton />
      ...
      <div className="flex flex-col items-center py-10 w-[19rem]">
        ...
        <button
          ...
          className="py-2 px-6 mt-6 bg-zinc-800 rounded-md border-transparent text-sm leading-6 hover:bg-zinc-700
            transition-colors focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 focus:ring-offset-zinc-900
          "
          onClick={props.restartFeedbackType}
        >
          Quero enviar outro
        </button>
      </div>
